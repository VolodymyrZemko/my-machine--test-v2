...
const trackMachineDetailView = (machineName) => {
  try {
    window.gtmDataObject?.push({
      ...
      local_event_action:'view machine page',
      local_event_label:`${machineName}`,
...
const trackUserManualClick = (machineName) => {
  try {
    window.gtmDataObject?.push({
      event: 'local_event', //as is, do not change!!
      local_event_category: 'machine assistance',
      local_event_action:'click on user manual',
      local_event_label:`${machineName}`,
      event_raised_by:'local_market_gr'
    });
  } catch (e) {}
...
const trackOverviewPDPClick = (machineName) => {
  try {
    window.gtmDataObject?.push({
      ...
      event: 'local_event', //as is, do not change!!
      local_event_category: 'machine assistance', 
      local_event_action:'overview - click to machine pdp',
      local_event_label:'learn more',
      event_raised_by:'local_market_gr'
    });
  } catch (e) {}
...
export function MachineDetail({ machine, onClose }) {
  ...
  const trackDetailTabClick = (machineName, tabName) => {
    try {
      window.gtmDataObject?.push({
        event: 'local_event', //as is, do not change!!
        local_event_category: 'machine assistance', 
        local_event_action:'click on tab',
        local_event_label:`${machineName} , ${tabName}`,    
        event_raised_by:'local_market_gr'
      });
    } catch (e) {}
  ...
  const trackGuideClick = (machineName, guideName) => {
    try {
      window.gtmDataObject?.push({
        event: 'local_event', //as is, do not change!!
        local_event_category: 'machine assistance', 
        local_event_action:'click on guide',
        local_event_label:`${machineName} - ${guideName}`,
        event_raised_by:'local_market_gr'
      });
    } catch (e) {}
  ...
  useEffect(() => {
    ...
    const url = `https://www.nespresso.com/shared_res/markets/gr/json/machine-assistance/v6/${machine.id}_${lang}.json`;
